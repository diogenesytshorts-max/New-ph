...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE}   🔧 FIXING WEB_CHALLAN_VIEW & LIVE DEPLOY (#129) ${NC}"
echo -e "${BLUE}====================================================${NC}\n"

# 1. Update web_challan_view.dart with clean types and correct provider
echo -e "${YELLOW}[1/3] Updating web_challan_view.dart...${NC}"
...
class _WebChallanViewState extends State<WebChallanView> with SingleTickerProviderStateMixin {
  ...
  Widget _buildRegisterTab(PharoahWebManager webPh) {
    ...
    double totalRegisterVal = filtered.fold(0.0, (s, c) => s + (c.totalAmount as num).toDouble());
...
git add .
git commit -m "Deploy Clean Challans Hub with Zero-Issues #PH-REV-129" || true
