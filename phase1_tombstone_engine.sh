...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE}   🛡️ PHASE 1: TOMBSTONE ENGINE (#PH-REV-127)      ${NC}"
echo -e "${BLUE}====================================================${NC}\n"

# 1. Update Top Bar Badge to #PH-REV-127 (Only in Top Bar next to Company Name)
echo -e "${YELLOW}[1/3] Updating web_top_bar.dart (#PH-REV-127)...${NC}"
...
# 2. Update pharoah_web_manager.dart with Persistent Tombstones Registry
echo -e "${YELLOW}[2/3] Adding Tombstone Anti-Resurrection Engine to pharoah_web_manager.dart...${NC}"
...
class PharoahWebManager with ChangeNotifier {
  ...
  // 🛡️ TOMBSTONE DELETION REGISTRY (Prevents Deleted Bills from Resurrecting)
  ...
  void _parseDownloadedFiles(Map<String, dynamic> files) {
    ...
    // 🛡️ Load Cloud Tombstones if available
    ...
    // Filter Out Any Deleted Records using Tombstones
  ...
  // ===========================================================================
  // ⚡ 2-WAY CLOUD PUSH WITH TOMBSTONES SNAPSHOT
  // ===========================================================================
  ...
  // ===========================================================================
  // ⚡ ACTION HOOKS (WITH INSTANT AUTO-SYNC & TOMBSTONE LOGIC)
  // ===========================================================================
...
# 3. Clean RAM and Compile Web Workstation
echo -e "${YELLOW}[3/3] Flushing RAM & Compiling Web Workstation...${NC}"
...
if [ ! -f "build/web/main.dart.js" ]; then
    echo -e "\n${RED}✖ Compilation failed. Check errors above.${NC}"
    exit 1
fi
echo -e "  ${GREEN}✔ Web compilation successful!${NC}\n"

# 4. Upload to Cloudflare Pages and Git Push
echo -e "${YELLOW}Uploading to Cloudflare Pages & Syncing Git...${NC}"
npx wrangler pages deploy build/web --project-name=pharoah-erp --commit-dirty=true

git add .
git commit -m "Phase 1: Tombstone Anti-Resurrection Engine & Deploy #PH-REV-127" || true
git push origin main || true

echo -e "\n${BLUE}====================================================${NC}"
echo -e "${GREEN}  🎉 PHASE 1 DEPLOYED: #PH-REV-127 LIVE!${NC}"
echo -e "${BLUE}====================================================${NC}"
