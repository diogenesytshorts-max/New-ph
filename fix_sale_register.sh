...
echo -e "${BLUE}====================================================${NC}"
echo -e "${BLUE}  🛠️ FIXING WEB SALE REGISTER & LIVE DEPLOY        ${NC}"
echo -e "${BLUE}====================================================${NC}\n"

# 1. Update web_top_bar.dart with single badge next to Company Name (#PH-REV-123)
echo -e "${YELLOW}[1/4] Setting single verification badge in Top Bar (#PH-REV-123)...${NC}"
...
# 2. Update WebSaleSummaryView with robust Date-Only matching & clean UI (No extra tags)
echo -e "${YELLOW}[2/4] Upgrading WebSaleSummaryView with normalized Date-Matching...${NC}"
...
# 3. Update WebNewSaleView to normalize Date and clean extra badges
echo -e "${YELLOW}[3/4] Normalizing bill save date in WebNewSaleView...${NC}"
...
# 4. Compile and Deploy to Cloudflare Pages & Git
echo -e "${YELLOW}[4/4] Compiling Web Bundle & Deploying Live...${NC}"
...
git add .
git commit -m "Fix Web Sale Register Normalized Date & Deploy #PH-REV-123" || true
git push origin main || true

echo -e "\n${BLUE}====================================================${NC}"
echo -e "${GREEN}  🎉 DEPLOYED SUCCESSFULLY: #PH-REV-123${NC}"
echo -e "${BLUE}====================================================${NC}"
